...
example (n m : ℤ) : span {n} ⊓ span {m} = span {lcm n m} := by
  ...
  · intro h
    ...
    obtain ⟨hn, hm⟩ := h
    simp at hn hm
    refine lcm_dvd ?h.mp.a.intro.hab ?h.mp.a.intro.hcb
    · have hn1 : ∃ hz : ℤ, hz * n = x := by exact Submodule.mem_span_singleton.mp hn
      obtain ⟨hz,hz1⟩ := hn1
      exact Dvd.intro_left hz hz1
    · have hm1 : ∃ hz : ℤ, hz * m = x := by exact Submodule.mem_span_singleton.mp hm
      obtain ⟨hz,hz1⟩ := hm1
      exact Dvd.intro_left hz hz1
  · intro h
    simp
    have hlcm : ∃ hlcm : ℤ, hlcm * (lcm n m) = x := by exact Submodule.mem_span_singleton.mp h
    obtain ⟨z, hz⟩ := hlcm
    constructor
    · refine mem_span_singleton.mpr ?h.mpr.intro.left.a
      exact dvd_trans (dvd_lcm_left n m) (Dvd.intro_left z hz)
    · refine mem_span_singleton.mpr ?h.mpr.intro.right.a
      exact dvd_trans (dvd_lcm_right n m) (Dvd.intro_left z hz)

...
-- #loogle ?a * _ < ?a * _ ↔ _    -- Both of the ?a have to be the same value
...
example (x : ℤˣ) : (x : ℤ) = 1 ∨ (x : ℤ) = -1 := sorry
...
-- unbundled would be (I: Set R) (hI : IsIdeal I)   - This does not exist
...
example {ι : Type*} [Fintype ι] (b : Basis ι K V) : V ≃ₗ[K] ι → K := by exact b.equivFun

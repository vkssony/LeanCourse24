...
lemma ill_L {z : ℂ }(l₁ l₂ : line): z ∈ l₁.points ∩ l₂.points ↔ ∃ i j : ℝ, (i:ℂ) * (l₁.z₁.re - l₁.z₂.re) + j* (l₂.z₂.re - l₂.z₁.re) = l₂.z₂.re - l₁.z₂.re ∧  i * (I*l₁.z₁.im - I*l₁.z₂.im) + j* (I*l₂.z₂.im - I*l₂.z₁.im) = I*l₂.z₂.im - I*l₁.z₂.im ∧ i * l₁.z₁ + (1 - i) * l₁.z₂ = z := by
  refine ⟨?_, ?_⟩ <;> intro h
...
lemma ilc_L  {z: ℂ} (c: Construction.circle) (l: line) (hc: 0 ≤ c.r): z ∈ c.points ∩ l.points ↔
    let a := (l.z₁.re - l.z₂.re)^2 - (I*l.z₁.im - I*l.z₂.im)^2
...
lemma quadratic_formula (x a b c : ℂ) (h:  a * x ^ 2 +  b * x + c = 0) (ha: a≠0): x = (-b + (b ^ 2 - 4 * c * a) ^ (1 / 2:ℂ)) / (2 * a) ∨ x = (-b - (b ^ 2 - 4 * c * a) ^ (1 / 2 :ℂ)) / (2 * a):= by
  ...
  rw [pow_two, quadratic_eq_zero_iff ha this] at h
  exact h

...
theorem dergree_two_eq_sqr :  Module.finrank K L = 2 ↔ ∃ x : E, x ^ 2 ∈ K ∧ ¬(x ∈ K) ∧ L = IntermediateField.adjoin K {x} := by sorry
...
lemma chain_iff_adjion_roots {z : ℂ}(M : Set ℂ) (h₀ : 0 ∈ M) (h₁ : 1 ∈ M): (∃n, ∃ α: Fin n → Set ℂ, z ∈ (succ_adjion (K_zero M) n α) ∧ succ_adjion_root (K_zero M) n α) ↔  (∃ (n : ℕ) (L : ℕ →  IntermediateField ℚ ℂ) (h: ∀i,  L i ≤ L (i + 1)),
...
lemma finrank_div  (h: L ≤ K): Module.finrank E L ∣ Module.finrank E K := by
  rw [dvd_iff_exists_eq_mul_left]
  use (Module.finrank L (extendScalars h))
  rw[mul_comm]
  symm
  exact Module.finrank_mul_finrank E ↥L ↥(extendScalars h)
end
...
lemma mulfinrank_help (R K L: IntermediateField ℚ ℂ) (h₁: R ≤ K) (h₂: K ≤ L) : Module.finrank R (extendScalars h₁) *
    Module.finrank (extendScalars h₁) (extendScalars h₂) = Module.finrank R (extendScalars (Trans.trans h₁ h₂)) := by
    --apply FiniteDimensional.finrank_mul_finrank R (extendScalars h₁) (extendScalars h₂)
    sorry

lemma degree_of_Ln {M : Set ℂ}(n : ℕ) (L : ℕ →  IntermediateField ℚ ℂ) (h:  ∃   (_: ∀i,  L i ≤ L (i + 1)),
  ...
  case succ n Ih =>
    ...
    symm
    have: Module.finrank ↥(L n) ↥(extendScalars hL) = Module.finrank ↥(extendScalars hn) ↥(extendScalars hL) := by norm_cast
    rw[this]
...
lemma Classfication_z_in_M_inf_2m {M : Set ℂ} (h₀: 0 ∈ M) (h₁:1 ∈ M) (z : ℂ) :
  ...
  have hm: ∃m, Module.finrank (K_zero M) ((K_zero M) ⟮z⟯) = 2^m := by
    have : Module.finrank (K_zero M) ((K_zero M) ⟮z⟯) ∣ 2^n := by
      rw[←hn]
      apply finrank_div
...
lemma Classfication_z_in_M_inf_2m_rat {M : Set ℂ} (h₀: 0 ∈ M) (h₁:1 ∈ M) (z : ℂ) (h: ∃j:ℕ, Module.finrank ℚ (K_zero M) = 2^j) :
  ...
  have:  (∃ m ,2 ^ m = (minpoly (K_zero M) z).degree) → ∃ m, 2 ^ m = (minpoly ℚ z).degree := by
    ...
    obtain ⟨m, hm⟩ := h'
    obtain ⟨j, hj⟩ := h
    have hmj: Module.finrank ℚ (restrictScalars ℚ ((K_zero M) ⟮z⟯)) = 2^(m+j) := by
      ...
      . by_contra h
        ...
        contradiction
    have hdiv: Module.finrank ℚ ℚ⟮z⟯ ∣ Module.finrank ℚ (restrictScalars ℚ ((K_zero M) ⟮z⟯)) := by
      apply finrank_div

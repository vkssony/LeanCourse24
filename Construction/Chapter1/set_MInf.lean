...
lemma L_M_inf_iff_M_M {l : line} (M: Set ℂ): l ∈ L (M_inf M) ↔ ∀ᶠ n in Filter.atTop, l ∈ L (M_I M n) := by
  unfold L
  simp
  constructor
  intro h
  ...
  intro h
  obtain ⟨n, hn⟩ := h
  have h: ∃ z₁ z₂, l ={z₁ := z₁, z₂ := z₂ : line} ∧ z₁ ∈ M_I M n ∧ z₂ ∈ M_I M n ∧ z₁ ≠ z₂ := by
    apply hn
    simp
  obtain ⟨z₁, z₂, h, hz₁, hz₂, Noteq⟩ := h
  use z₁
...
lemma C_M_inf_iff_M_M {c :circle} (M: Set ℂ): c ∈ C (M_inf M) ↔ ∀ᶠ n in Filter.atTop, c ∈ C (M_I M n) := by
  unfold C
  simp
  constructor
  intro h
